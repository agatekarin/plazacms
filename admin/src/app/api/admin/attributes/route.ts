...
export async function GET() {
  ...
  const valuesByAttr = vals.rows.reduce((acc: Record<string, any[]>, v: any) => {
    (acc[v.attribute_id] ||= []).push({ id: v.id, value: v.value });
    return acc;
  }, {});

  const items = attrs.rows.map((a: any) => ({ id: a.id, name: a.name, values: valuesByAttr[a.id] || [] }));
...
export async function POST(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to create attribute", detail: err?.message || "DB error" }, { status: 500 });
  } finally {
    client.release();
  }
}
