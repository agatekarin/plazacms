...
export default function SignInPage() {
  ...
  // Fetch CSRF token from Auth.js (Hono) so the login POST includes it
  useEffect(() => {
    const loadCsrf = async () => {
      try {
        const res = await fetch("/api/authjs/csrf", { credentials: "include" });
        if (res.ok) {
  ...
  // Compute absolute callback URL to ensure redirect lands back on admin domain
  useEffect(() => {
    try {
  ...
  // Using a standard form POST ensures top-level navigation so Set-Cookie is not blocked
  const onSubmit = undefined as any;
      ...
      <div className="mx-auto max-w-md px-6 py-16">
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
            Sign in to your PlazaCMS admin
          </p>
        </div>
